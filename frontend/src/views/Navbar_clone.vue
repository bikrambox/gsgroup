  ...
  <Disclosure as="nav"
          ...
          <div class="hidden sm:ml-6 sm:block">
            <div class="flex space-x-4">
              <template v-for="item in navigation" :key="item.name">
                <!-- Regular menu items -->
                <a v-if="!item.hasDropdown" :href="item.href" @click="handleItemClick(item.name)"
                ...
                <Menu v-else as="div" class="relative inline-block text-left">
                  <div @mouseenter="hoveredDropdown = item.name" @mouseleave="hoveredDropdown = null">
                    <MenuButton
                    ...
                    <transition enter-active-class="transition ease-out duration-100"
                      ...
                      <MenuItems v-show="hoveredDropdown === item.name" static
                        class="absolute left-0 z-10  w-48 origin-top-right bg-gray-200 dark:bg-gray-900 text-gray-700 dark:text-gray-50 focus:outline-none">
                        <div class="">
                          <MenuItem v-for="subItem in item.subItems" :key="subItem.name" v-slot="{ active }">
                          <a :href="subItem.href" @click.prevent.stop="handleItemClick(subItem.name)"
    ...
    <DisclosurePanel class="sm:hidden absolute top-16 inset-x-0 z-50 bg-gray-50 dark:bg-gray-800 shadow-md">
      <div class="space-y-1 px-2 pt-2 pb-3">
        <template v-for="item in navigation" :key="item.name">
          <!-- Regular menu items -->
          <button v-if="!item.hasDropdown"
            @click.prevent.stop="handleItemClick(item.name)"
            :class="[item.current ? 'bg-gray-300 dark:bg-gray-700 text-gray-900 dark:text-white' : 'text-gray-900 dark:text-gray-50 hover:bg-gray-200 dark:hover:bg-gray-700', 'block w-full text-left rounded-md px-3 py-2 text-base font-medium']"
            :aria-current="item.current ? 'page' : undefined">
            {{ item.name }}
          </button>
          ...
          <div v-else class="space-y-1">
            <button @click.stop="item.isOpen = !item.isOpen"
            ...
            <div v-show="item.isOpen" class="ml-3">
              <button v-for="subItem in item.subItems" :key="subItem.name"
                @click.prevent.stop="handleItemClick(subItem.name)"
